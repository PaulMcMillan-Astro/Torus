...
void IsoMap::psifunc(const double p, double& f, double& df) const
{
    psi  = p;
    spsi = sin(psi);
    cpsi = cos(psi);
...
// solves  tr = psi - eps * sin(psi);    for psi.
{
    ...
    if(ps<Pi) ps = rtsafe(this,&IsoMap::psifunc,ps,Pi,tol);
	else  ps = rtsafe(this,&IsoMap::psifunc,Pi,ps,tol);
    if(ps!=psi) {
        psi  = ps;
        spsi = sin(psi);
        cpsi = cos(psi);
...
double IsoMap::catan(const double& aa) const
...
double IsoMap::wfun(const double& fac) const
{
...
double IsoMap::wfundw(const double& fac, double& dwdpsi) const
{
...
double IsoMap::Fint(double p, double q, double Pc, double Qc) const
...
vec4 IsoMap::lower_bounds(const double x, const double v) const
{
    return -upper_bounds(x,v);
}

vec4 IsoMap::upper_bounds(const double x, const double v) const
{
...
void IsoMap::Set()
{
    M = gamma*gamma;
    b = beta*beta;
    if(b==0.) TorusError("IsoMap: scale radius = 0 not allowed",2);
    if(M==0.) TorusError("IsoMap: mass = 0 not allowed",2);
    if(b>0. && M>0.) {
	sMb  = sqrt(M*b);
	sMob = sMb/b;
	jp   = fabs(Lz)/sMb;
...
IsoMap::IsoMap(const IsoPar& p)
{ 
    set_parameters(p);
    Set();
}

IsoMap::IsoMap(const IsoMap& I): gamma(I.gamma),beta(I.beta),Lz(I.Lz),r0(I.r0)
{ 
    Set();
}

PSPD IsoMap::ForwardWithDerivs(const PSPD& JT, double dQdT[2][2]) const
...
// Extract and scale the actions and angles.
    ...
    if(jr<0. || jt<0.|| this->jp<0.) {
        TorusError("IsoMap: negative action(s)",2); 
	return PSPD(0.);
    }
// Set up auxilliary variables independent of the angles tr and tt.
    ...
    if(at==0.) e = 1.;
        ...
        e2    = 1. - pow(at/a,2)/H2;
        e     = (e2>0) ? sqrt(e2) : 0;
        if(e2<-1.e-3) TorusError("IsoMap: bad e in ForwardWithDerivs",2);
    }
    ae  = a*e;
    eps = ae*H2;
    wr  = H2*HH;
...
PSPD IsoMap::ForwardWithDerivs(const PSPD& JT, double dQdT[2][2], 
...
void IsoMap::Derivatives(double dQPdJ[4][2]) const
...
{
  if(!derivs_ok)
      TorusError(" `IsoMap::Derivatives()' called without For/Backward",-2);
...
void IsoMap::Derivatives(double dQPdJ[4][2], Pdble dQPdA[4]) const
// Calculates the derivatives of the spherical phasespace co-ordinates w.r.t.
...
PSPD IsoMap::Forward(const PSPD& JT) const
// Transforms action-angle variables (JT) to phasespace co-ordinates (r,theta)
...
// Set up auxilliary variables independent of the angles tr and tt.
    ...
    if(at==0.) e = 1.;
        ...
        e2    = 1. - pow(at/a,2)/H2;
        e     = (e2>0) ? sqrt(e2) : 0;
        if(e2<-1.e-3) TorusError("IsoMap: bad e in Forward",2);
    }
    ae  = a*e;
    eps = ae*H2;
    wr  = H2*HH;
...
PSPD IsoMap::Backward(const PSPD& QP) const
...
// Perform some consistency checks
    if(csth==0.&&jp!=0.) {
	TorusError("IsoMap: Jp!=0 && R=0 in Backward",2);
	return PSPD(0.);
    }
    if(r==0. && pt!=0.) {
	TorusError("IsoMap: pt!=0 && r=0 in Backward",2);
	return PSPD(0.);
    }
// Set up auxialiary variables
    ...
    H2 += 2./(u+2.);
    if(H2<=0.) {
	TorusError("IsoMap: H>=0 in Backward",2);
	return PSPD(0.);
    }
    H   =-0.5*H2;
    HH  = sqrt(H2);
...
////////////////////////////////////////////////////////////////////////////////
PSPT IsoMap::Backward3D(const PSPT& QP3) const
{ // For the 3D case
...
////////////////////////////////////////////////////////////////////////////////
PSPT IsoMap::Forward3D(const PSPT& Jt3) const
{ // For the 3D case
