...
********************************************************************************
*                                                                              *
* class IsoMap     class for the mapping between action-angle variables and    *
...
*                  for fitting algorithms. Therefore, class IsoMap used the    *
...
inline void IsoMap::set_parameters(const IsoPar& p)
{ 
    gamma = p(0);
    beta  = p(1);
    Lz    = p(2);
...
inline vec4 IsoMap::parameters() const
{
    double a[4] = {gamma,beta,Lz,r0};
    return vec4((const double*)a); 
}

inline void IsoMap::parameters(vec4* p) const
{
	(*p)[0] = gamma;
	(*p)[1] = beta;
	(*p)[2] = Lz;
