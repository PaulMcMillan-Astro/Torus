...
//------------------------------------------------------------------------------
//
// The class PoiCh_tr contains routines which implement this point transform
...
//------------------------------------------------------------------------------
 public:
  PoiCh_tr();                                           // various constructors
  PoiCh_tr(Actions, Cheby, Cheby, Cheby, double,double);// various constructors
  PoiCh_tr(const double*);                              // various constructors
  PoiCh_tr(Potential *, const Actions);                 // various constructors
  ~PoiCh_tr() {}
...
inline void PoiCh_tr::set_parameters(Actions J, Cheby Ch1, Cheby Ch2, Cheby Ch3,
				     double thmx, double om)
{
  Jl = J(1);
  Lz = fabs(J(2));
...
inline int PoiCh_tr::NumberofParameters() const {
  return 7 + xa.NChb + ya.NChb + za.NChb; // 2 actions, thmax, 3*NCheb +.
}

inline void PoiCh_tr::parameters(double *tmp) const {
