...
//------------------------------------------------------------------------------
double PoiClosedOrbit::get_x(double th,double &dx,double &d2x) const{
	double xpp,th2=th*th;
...
double PoiClosedOrbit::get_z(double th,double &dz,double &d2z) const{
	double zpp,th2=th*th;
...
vec2 PoiClosedOrbit::chebderivs(const double psi, const vec2 yz) {
  double t = thmax * sin(psi), t0 = yz[0]*yz[1], tiny=1.e-20;
  vec2 dyzdt;
  double tvr    = vr2.unfit1(psi), tdrdth = drdth2.unfit1(psi);
  double tpth   = pth2.unfit1(psi*psi);
  if(psi == 0.) tdrdth = 0.;  // symmetry, should be firmly stated
  double tmp = alt2-pow(Lz/cos(t0),2);
  double ptho = (tmp>tiny)? sqrt(tmp) : sqrt(tiny);
...
void PoiClosedOrbit::set_parameters(Potential *Phi, const Actions J, ToyMap *TM) {
	...
	IsoPar IP=TM->parameters();
	Lz = fabs(IP[2]); alt2=pow(Lz+Jl,2);
	//Phi->set_Lz(J(2));
...
//	ToyMap *TM = new ToyIsochrone(IP);
	...
	double a_toy=qp(0); printf("Toy shell: %f",a_toy);
	IP[3]-=a_toy-tbr[0]; TM->set_parameters(IP);//will reduce a_toy to true shell radius
	qp=TM->Forward(JT); printf(" %f\n",qp(0));
	a_toy=tbr[0];
	printf("PC IP: %f %f %f %f %f %f\n",IP[0],IP[1],IP[2],IP[3],Lz,Jl);
	for(int i=0; i!=imax; i++){
		tbir[i] *=a_toy;
		tbth2[i]   = tbth[i]*tbth[i];
	}
	...
	double thmax2 = acos(sqrt(Lz*Lz/alt2));//toy theta_max
	int many=100000;
	double tpsi,*thet;
	...
	rr[0] = 0.;  yy[0] = .7*yy[1];
	rr[nr-1] = 2*rr[nr-2];  yy[nr-1] = 1.3*yy[nr-2];
  ...
  // extend zz to all theta<pi/2 and fit to theta*(apoly in theta**2)
	...
	thmax *=.9;// we avoid evaluating derivative at end point of fit
  ...
  // at th = pi/2
	...
	psi[0]=ya.unfit1(rr[nr-2]);
	psi[1]=za.unfit1(pow(1.1*thmax,2));
...
void PoiClosedOrbit::set_parameters(const double* param) {
  int ncx,ncy,ncz;
  //Jl = param[0]; Lz = param[1]; omz = param[3];
  thmax = param[0];
  ncx = int(param[2]);
  double chx[ncx];
  for(int i=0;i!=ncx;i++) chx[i] = param[3+i];
  ncy = int(param[3+ncx]);
  double chy[ncy];
  for(int i=0;i!=ncy;i++) chy[i] = param[4+ncx+i];
  ncz = int(param[4+ncx+ncy]);
  double chz[ncz];
  for(int i=0;i!=ncz;i++) chz[i] = param[5+ncx+ncy+i];
...
void PoiClosedOrbit::parameters(double *tmp) const {
	int ncx = xa.NChb, ncy = ya.NChb, ncz = za.NChb;
	//tmp[0] = Jl; tmp[1] = Lz;
	tmp[0] = thmax;// tmp[3] = omz;
	tmp[2] = ncx;
	for(int i=0;i!=ncx;i++) tmp[3+i] = xa.e1[i];
	tmp[3+ncx] = ncy;
	for(int i=0;i!=ncy;i++) tmp[4+ncx+i] = ya.e1[i];
	tmp[4+ncx+ncy] = ncz;
	for(int i=0;i!=ncz;i++) tmp[5+ncx+ncy+i] = za.e1[i];
...
int PoiClosedOrbit::NumberofParameters() const {
	return 5 + xa.NChb + ya.NChb + za.NChb;
}
...
///////////////////////////////////////////////////////////////////////////////
PoiClosedOrbit::PoiClosedOrbit(Potential *Phi, const Actions J, ToyMap *TM) {
  set_parameters(Phi,J,TM);
}
...
{//from toy to real coordinates
  ...
  do {
    tmpint++;
    if(tmpint>99) printf("PoiClosedOrbit::Forward: convergence failure %f %f\n",tmptht,qp(1));
...
/*void PoiClosedOrbit::plot_grid(int np,double rmin,double rmax,double a_toy,double thmax2,
	...
	double R[MAX(np,imax)],z[MAX(np,imax)],Thmax=1;
	for(int i=0;i<imax;i++){
	...
	for(int i=0;i<np;i++){
		...
		for(int j=0;j<np;j++){
			double th=-Thmax+2*j*Thmax/(double)(np-1);
			PSPD qp(r,th,0,0), QP=Forward(qp);
			R[j]=QP[0]; z[j]=QP[1];
//			printf("(%f %f) (%f %f)\n",R[j],z[j],r,th);
		}
		connect(R,z,np);
	...
	for(int j=0;j<np;j++){
		double th=-j*thmax2/(double)(np-1);
		PSPD qp(a_toy,th,0,0), QP=Forward(qp);
		R[j]=QP[0]; z[j]=QP[1];
	}
	setcolour("red"); connect(R,z,np); setcolour("black");
	for(int j=0;j<np;j++){
		double th=-Thmax+2*j*Thmax/(double)(np-1);
		for(int i=0;i<np;i++){
			double r=rmin+i*(rmax-rmin)/(double)(np-1);
...
void PoiClosedOrbit::plot_SoS(double *th,double *pth,int imax,double thmax2,ToyMap *TM){
	plots(imax,th,pth,0,1,-1,1,"\\gq",3,"p\\d\\gq",7,-.9,10);
	relocate(.6,.9);draw(.75,.9);label(5," true");
	Angles A; A[0]=0;
	int np=100; double thp[np],pthp[np];
	for(int i=0;i<np;i++){
	...
	setltype(2); setcolour("red"); connect(thp,pthp,np);
	relocate(.6,.7);draw(.75,.7);label(11," mapped toy");
	setcolour("blue");
	for(int i=0;i<np;i++){
		thp[i]=i*thmax2/(double)np;
		pthp[i]=-sqrt(alt2-pow(Lz/cos(thp[i]),2));
	}
	connect(thp,pthp,np);
	relocate(.6,.8);draw(.75,.8);label(4," toy");
	grend(1);
}
*/
