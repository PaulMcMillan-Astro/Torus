...
// Numbers and routine needed for external integration routines ----------------
//double thmaxforactint;
//Cheby vr2, drdth2, pth2;

double PoiCh_tr::actint(double theta) const {
...
void PoiCh_tr::do_orbit(PSPD clo,   double delt, Potential* Phi,
...
void PoiCh_tr::set_Rstart(double& Rstart,double Rstop, double& odiff,double& dr,
...
void PoiCh_tr::set_E(const double tJl,double& odiffJ,double& E, 
...
void PoiCh_tr::RewriteTables(const int n, double *time, double *tbR,double *tbz,
...
vec2 PoiCh_tr::chebderivs(const double psi, const vec2 yz) {
...
double PoiCh_tr::stepper(vec2 &tyz,vec2 &dyzdt,const double psi,const double h) 
{
...
void PoiCh_tr::yzrkstep(vec2 &yz, vec2 &dyzdt, const double tol, double& psi,
...
double PoiCh_tr::yfn(double t, vec2 * ys,const int which,double * thet, int n) 
{
  int klo=0,khi=n-1,k;
  while(khi-klo > 1) {
    k=(khi+klo)/2;
...
void PoiCh_tr::set_parameters(Potential *Phi, const Actions J) {
  Jl = J(1);
  Lz = fabs(J(2));
  alt2=(fabs(Lz)+Jl)*(fabs(Lz)+Jl);
  Phi->set_Lz(Lz);
  if(Jl<=0.) { 
    cerr << "PoiCh_tr called for Jl <= 0. Not possible.\n";
    return;
  }
  bool first=true,firstE=true, either_side=false, es_Jl=false, done=false;
  int Nt=1024,np=0,norb=0,nE=0,nEmax = 50,imax, NCheb=10;
  ...
  for(nE=0;!done && nE!=nEmax;nE++) {           // iterate energy 
    ...
    tJl = 2.*qromb(this,&PoiCh_tr::actint,0,thmax)/Pi;
...
PoiCh_tr::PoiCh_tr(const double* param) {
  set_parameters(param);
}

void PoiCh_tr::set_parameters(const double* param) {
  int ncx,ncy,ncz;
...
PoiCh_tr::PoiCh_tr(Actions J, Cheby ch1, Cheby ch2, Cheby ch3, 
...
///////////////////////////////////////////////////////////////////////////////
PoiCh_tr::PoiCh_tr(Potential *Phi, const Actions J) {
...
PSPD    PoiCh_tr::Forward           (const PSPD &qp)                const 
{
...
PSPD   PoiCh_tr::Backward           (const PSPD &QP)                const 
{
  PSPD qp; 
...
////////////////////////////////////////////////////////////////////////////////
PSPT PoiCh_tr::Forward3D(const PSPT& w3) const
{
  PSPT W3 = w3;
...
////////////////////////////////////////////////////////////////////////////////
PSPT PoiCh_tr::Backward3D(const PSPT& W3) const
{
  PSPT w3 = W3;
  PSPD W2 = W3.Give_PSPD();
...
void    PoiCh_tr::Derivatives(double dQPdqp[4][4]) const {
