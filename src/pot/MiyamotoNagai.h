...
class MiyamotoNagai : public Potential {
public:
  const double A,ABoB,Bq,ABq,GM;

  //public:
    MiyamotoNagai(const double=1., const double=1., 
	       const double=1.);
    ...
    friend ostream& operator<< (ostream&, const MiyamotoNagai&);
};

inline MiyamotoNagai::MiyamotoNagai(const double m, const double a,
				    const double b) 
