...
//double StackelPot::findG(const double tau)
//{
...
//double StackelPot::findG(const double tau, double& dG)
//{
...
double StackelPot::operator() (const double R, const double z) const
{
  if(alpha==gamma)  {    
...
double StackelPot::operator() (const double R, const double z, double& dPdR,
			       double& dPdz) const
{
  if(alpha == gamma)  {
    double r=sqrt(R*R + z*z), ia= sqrt(-1./alpha);
...
//double StackelPot::operator() (const double R, 
//                           double& dPdR, double& d2PdRR) const
//{

//}

//double StackelPot::operator() (const double R, const double z,
...
double StackelPot::RfromLc(const double L, double* dR) const
{
  bool more=false;
...
double StackelPot::LfromRc(const double R, double* dR) const
{
  double dPR,dPz,P;
  P = (*this)(R,0.,dPR,dPz);
  return sqrt(R*R*R*dPR);  
}
Frequencies StackelPot::KapNuOm(const double R) const
{
...
ostream& operator<< (ostream& to, const StackelPot& P)
{
