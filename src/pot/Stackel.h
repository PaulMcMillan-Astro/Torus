...
class StackelPot : public Potential {
private:
...
public:
    StackelPot(const double=1., const double=1., 
	       const double=1.);
    ...
    friend ostream& operator<< (ostream&, const StackelPot&);
};

inline StackelPot::StackelPot(const double rho0, const double Rs,
			      const double q) 
