...
#include "Pspline.h"
 ...
 void SwallowRestofLine(istream& from)
{
    char c;
...
double SpheroidDensity::mass_integrand(const double y) const
{
  if(rci) {
    ...
    return pow(y,2.-gam) * pow(y1,bet-4.) * exp(-(m*m*rci*rci));
  }
